...
class PublicUserApiTests(TestCase):
    ...
    def test_create_user_with_email_exists(self):
        """ Test creating user with email exists. """
        payload = {
            'email': 'test@example.com',
            'password': 'testpass123',
            'name': 'Test Name',
    ...
    def test_passwortd_too_short(self):
        ...
        res = self.client.post(CREATE_USER_URL, payload)
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
        user_exixsts = get_user_model().objects.filter(email=payload['email']).exists()
        self.assertFalse(user_exixsts)
    ...
    def test_create_token_bad_credentials(self):
        ...
        create_user(email='test@exmple.com', password='testpass123')
        payload = {
...
class PrivateUserApiTests(TestCase):
    ...
    def test_retrieve_profile_success(self):
        """ Test retrieving propfie for logged in user.  """
        res = self.client.get(ME_URL)
        self.assertEqual(res.status_code, status.HTTP_200_OK)
